...
inline bool BVHNode::Hit(const Ray& r, double tMin, double tMax, HitRecord& record, int id, int depth) const
{
	if (!m_box.Hit(r, tMin, tMax)) return false;

	bool hitLeft = m_leftChild->Hit(r, tMin, tMax, record, id, depth);
	bool hitRight = m_rightChild->Hit(r, tMin, (hitLeft ? record.t : tMax), record, id, depth);
