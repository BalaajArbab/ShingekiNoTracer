...
inline bool RotateY::Hit(const Ray& r, double tMin, double tMax, HitRecord& record, int id, int depth) const
{
	...
	if (!m_object->Hit(rotatedRay, tMin, tMax, record, id, depth)) return false;
...
inline bool RotateX::Hit(const Ray& r, double tMin, double tMax, HitRecord& record, int id, int depth) const
{
	Point3 origin = r.Origin();
	Point3 direction = r.Direction();
...
inline bool RotateZ::Hit(const Ray& r, double tMin, double tMax, HitRecord& record, int id, int depth) const
{
	Point3 origin = r.Origin();
	Point3 direction = r.Direction();
